...
def update_dqn(q, q_target, optimizer, gamma,
               obs, act, rew, next_obs, tm, is_double_dqn, is_distributional, is_dueling_dqn, num_atoms, v_min, v_max):
    """
    Update the DQN, Double DQN, or Dueling DQN network for one optimizer step using the target network.
    """
    optimizer.zero_grad()
...
def animate(env, agent, is_noisy_nets, is_distributional, is_double_dqn, is_dueling_dqn, filename="trained.gif", max_steps=10_000):
    ...
    def noisy_policy(obs: torch.Tensor):
        return (agent.q if not is_dueling_dqn else agent.dueling_agent.q)(obs).argmax(dim=1).detach().numpy()[0]

    imgs = rendered_rollout(
        noisy_policy if is_noisy_nets 
        else (lambda obs: agent.dueling_agent.q(obs).argmax(dim=1).detach().numpy()[0]) if is_dueling_dqn
